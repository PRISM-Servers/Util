...
export function Ping(msg: Discord.Message, color: Discord.ColorResolvable) {
    ...
    msg.channel.send({embeds: [embed]}).then(msg2 => {
        ...
        const took = final / BigInt("1000000");
            
        const embed = new Discord.MessageEmbed(msg2.embeds[0]);
...
export function FindMember(guild: Discord.Guild, name: string) {
    return new Promise((resolve, reject) => {
        ...
        guild.members.fetch().then(members => {
            const found = members.filter(x => x.id == name || x.user.tag.toLowerCase() == name || x.user.username.toLowerCase() == name);
...
export function GetMessageResponse(msg: Discord.Message, time: number = 15 * 1000, accept_args: string[] = [], text: string = "", cleanup: boolean = true) {
    ...
    return new Promise(async (resolve, reject) => {
        ...
        const sent = text ? await msg.channel.send(text) : null;
        const collector = msg.channel.createMessageCollector({filter: m => m.author.id == msg.author.id, time: time});
...
export function SendWebhookMessage(message: string, url: string, avatar: string, name: string) {
    ...
    const _url = url.replace("https://discord.com/api/webhooks/", "");
    const split = _url.split("/");

    if (split.length < 2) return false;

    const client = new Discord.WebhookClient({url});
...
export function HandleEval(msg: Discord.Message, res: string, color: Discord.ColorResolvable = "ORANGE", alreadySent?: Discord.Message, _code?: string) {
    ...
    const code = msg.content && typeof msg.content == "string" ? getArgsFromMessage(msg.content) : ([undefined, _code] as string[]);
    code.shift();
    let codeJoined = code.join(" ");

    if (codeJoined.toLowerCase().startsWith("eval")) codeJoined = codeJoined.substring(4);
    codeJoined = codeJoined.trim();
    ...
    else msg.channel.send({files: [{name: "eval_" + logFormat() + (isValidJSON(res) ? ".json" : ".txt"), attachment: Buffer.from(res)}]});
}

export function HandleEvalInteract(res: unknown, code: string, interaction: unknown, color: Discord.ColorResolvable = "ORANGE") {
    if (isObject(res) || Array.isArray(res)) {
        try { res = JSON.stringify(res, null, 2); }
        catch (_) { /**/ }
    }
    ...
    if (res.length < 1000) {
        ...
        embed.addFields({name: "Response", value: "```\nCheck file```"});
        // @ts-expect-error idk
        files.push(new Discord.MessageAttachment(Buffer.from(res), "eval_" + logFormat() + (isValidJSON(res as string) ? ".json" : ".txt")));
    }
...
export function LogRejection(error: Error | string, client: Discord.Client, webhook: string) {
    const temp = JSON.stringify(error, null, 2);
...
export function LogException(error: Error | string, client: Discord.Client, webhook: string) {
    ...
    const user = client?.user?.username ? client?.user?.username : client && typeof client == "string" ? client : "Utils"; 
